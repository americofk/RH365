...
// Ruta: TS/Jobs/jobs-dt.ts
// Descripción:
//   - Lista de Cargos con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// Estándar: ISO 27001 - Gestión de información sensible
// ============================================================================
...
(function () {
    ...
    const pageContainer = d.querySelector("#jobs-page");
    ...
    const $table: any = $("#jobs-table");
    if (!$table.length) return;

    let jobsData: JobRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['RecID', 'JobCode', 'Name', 'JobStatus', 'CreatedOn'];
    ...
    const getColumnsFromData = (sample: JobRow): string[] => {
        if (!sample || typeof sample !== 'object') {
            return [...defaultColumns];
        }
        const excluded = new Set(['RowVersion', 'DataareaID']);
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: JobRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: JobRow[] }) => void) => {
                loadJobs().then(items => {
                    jobsData = items;
                    callback({ data: items });
    ...
    const loadJobs = async (): Promise<JobRow[]> => {
        ...
        const response: JobResponse = await fetchJson(url);
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('jobs-summary');
        if (summary) {
            summary.textContent = `${count} cargo${count !== 1 ? 's' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const rows: JobRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `Cargos_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteJob = async (recId: string): Promise<void> => {
        const url = `${apiBase}/Jobs/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar cargo: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/Job/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/Job/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ...
            : `¿Está seguro de eliminar ${count} cargos?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteJob(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Cargo(s) eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar cargo(s)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/Jobs?pageNumber=1&pageSize=1`;
            const probe: JobResponse = await fetchJson(probeUrl);
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "Jobs", userRefRecID, dataareaId);
