...
(function () {
    ...
    if (!$) { console.error('jQuery no encontrado'); return; }
    ...
    const $table: any = $("#educationlevels-table");
    ...
    const getColumnsFromData = (sample: EduRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    defaultContent: '',
                    render: (_d: any, _t: string, row: EduRow) =>
            ...
            ajax: (_rq: any, callback: (r: { data: EduRow[] }) => void) => {
                loadEducationLevels()
                    .then(items => {
                        dataRows = items;
                        callback({ data: items });
    ...
    const loadEducationLevels = async (): Promise<EduRow[]> => {
        const url = `${apiBase}/EducationLevels?pageNumber=1&pageSize=100`;
        const response: PagedResponse<EduRow> | EduRow[] = await fetchJson(url);
    ...
    const deleteEducationLevel = async (recId: string): Promise<void> => {
        const url = `${apiBase}/EducationLevels/${recId}`;
        const headers: Record<string, string> = token ? { 'Authorization': `Bearer ${token}` } : {};
        const r = await fetch(url, { method: 'DELETE', headers });
        if (!r.ok) throw new Error(`Error al eliminar nivel educativo: ${r.status}`);
    };
    ...
    const updateViewsDropdown = (): void => {
        try {
            ...
            views.forEach((v: any) => {
                ...
                // Cargar vista
                $item.find('a[data-view-id]').on('click', async (e) => {
                    e.preventDefault();
                    await loadViewById(v.RecID);
                });
                ...
                $item.find('.btn-set-default').on('click', (e) => {
                    ...
                    (w as any).ALERTS?.confirm?.(
                        `¿Establecer "${v.ViewName}" como predeterminada?`,
                        'Confirmar',
                        async (ok: boolean) => {
                            if (!ok) return;
    ...
    $('#btn-edit').on('click', () => {
        ...
        if ($c.length) window.location.href = `/EducationLevel/NewEdit?recId=${$c.data('recid')}`;
    });
        ...
        (w as any).ALERTS?.confirm?.(msg, 'Confirmar Eliminación', async (ok: boolean) => {
            ...
            try {
                ...
                $checked.each(function () {
                    ...
                    if (id) ops.push(deleteEducationLevel(String(id)));
                });
                await Promise.all(ops);
                (w as any).ALERTS?.ok?.('Registro(s) eliminado(s) correctamente', 'Éxito');
                $table.DataTable().ajax.reload();
            } catch (err) {
                console.error('Error al eliminar:', err);
                (w as any).ALERTS?.error?.('Error al eliminar registro(s)', 'Error');
            }
        }, { type: 'danger' });
    });
    ...
    $table.on('click', 'tbody tr', function (e: any) {
        if ($(e.target).is('input, label, a, button, .icheckbox_flat-green')) return;
    ...
    $table.on('dblclick', 'tbody tr', function (e: any) {
        if ($(e.target).is('a,button,input,label,.icheckbox_flat-green,*[role="button"]')) return;
        ...
        const data = dt.row($tr).data() as any;
        const id = data?.RecID ?? $tr.find('input.row-check').data('recid');
        if (id) window.location.href = `/EducationLevel/NewEdit?recId=${id}`;
    });
    ...
    $('#btn-reset-view').on('click', () => {
        (w as any).ALERTS?.confirm?.('¿Restablecer a la vista por defecto?', 'Confirmar', (ok: boolean) => {
            if (!ok) return;
    ...
    $(async function () {
        try {
            ...
            const probe: PagedResponse<EduRow> | EduRow[] = await fetchJson(probeUrl);
