// ============================================================================
// Archivo: coursetypes-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/CourseTypes/coursetypes-dt.ts
// Descripción:
//   - Lista de Tipos de Curso con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Compatible con ambos formatos de respuesta del API
// Estándar: ISO 27001 - Presentación segura de datos
// ============================================================================

type CourseTypeRow = Record<string, unknown>;
...
(function () {
    ...
    const pageContainer = d.querySelector("#coursetypes-page");
    ...
    const $table: any = $("#coursetypes-table");
    if (!$table.length) return;

    let courseTypesData: CourseTypeRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'CourseTypeCode', 'Name', 'Description', 'CreatedOn'];
    ...
    const getColumnsFromData = (sample: CourseTypeRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: CourseTypeRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: CourseTypeRow[] }) => void) => {
                loadCourseTypes().then(items => {
    ...
    const loadCourseTypes = async (): Promise<CourseTypeRow[]> => {
        const url = `${apiBase}/CourseTypes?pageNumber=1&pageSize=100`;
        const response: any = await fetchJson(url);

        // Formato correcto: objeto con propiedad Data
        if (response && typeof response === 'object' && 'Data' in response && Array.isArray(response.Data)) {
            return response.Data;
        }

        // Formato alternativo: array directo (compatibilidad)
        if (Array.isArray(response)) {
            /*console.warn('⚠️ API devolviendo array directo');*/
            return response;
        }
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('coursetypes-summary');
        if (summary) {
            summary.textContent = `${count} tipo${count !== 1 ? 's' : ''} de curso`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const fileName = `TiposCurso_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteCourseType = async (recId: string): Promise<void> => {
        const url = `${apiBase}/CourseTypes/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar tipo de curso: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/CourseType/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/CourseType/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ? '¿Está seguro de eliminar este tipo de curso?'
            : `¿Está seguro de eliminar ${count} tipos de curso?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteCourseType(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Tipo(s) de curso eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar tipo(s) de curso', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/CourseTypes?pageNumber=1&pageSize=1`;
            const probe: any = await fetchJson(probeUrl);

            let sampleData: CourseTypeRow | null = null;

            if (probe && typeof probe === 'object' && 'Data' in probe && Array.isArray(probe.Data) && probe.Data.length > 0) {
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "CourseTypes", userRefRecID, dataareaId);
