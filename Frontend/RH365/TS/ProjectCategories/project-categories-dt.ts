...
type ProjectCategoryRow = Record<string, unknown>;

interface ProjectCategoryResponse {
    Data: ProjectCategoryRow[];
    TotalCount: number;
    PageNumber: number;
    PageSize: number;
    TotalPages: number;
...
(function () {
    ...
    const $table: any = $("#project-categories-table");
    if (!$table.length) return;

    let categoriesData: ProjectCategoryRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'CategoryName', 'LedgerAccount', 'ProjectRefRecID', 'ProjectCategoryStatus', 'CreatedOn'];
    ...
    const getColumnsFromData = (sample: ProjectCategoryRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: ProjectCategoryRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: ProjectCategoryRow[] }) => void) => {
    ...
    const loadCategories = async (): Promise<ProjectCategoryRow[]> => {
        const url = `${apiBase}/ProjectCategories?pageNumber=1&pageSize=100`;
        const response: ProjectCategoryResponse = await fetchJson(url);
    ...
    const exportToCSV = (): void => {
        ...
        const rows: ProjectCategoryRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
    ...
    const deleteCategory = async (recId: string): Promise<void> => {
        ...
        (w as any).ALERTS.confirm(
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    (w as any).ALERTS.ok('Categoría(s) eliminada(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    (w as any).ALERTS.error('Error al eliminar categoría(s)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            ...
            const probe: ProjectCategoryResponse = await fetchJson(probeUrl);
            ...
            visibleColumns = [...defaultColumns];
            initializeDataTable(visibleColumns);
            (w as any).ALERTS.error('Error al cargar la configuración', 'Error');
        }
    });
})();