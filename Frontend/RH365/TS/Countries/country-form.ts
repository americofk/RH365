...
(function () {
     ...
     */
    const loadCountryData = async (): Promise<void> => {
        if (isNew) {
    ...
    const saveCountry = async (): Promise<void> => {
        ...
        try {
            ...
            (w as any).ALERTS.ok(
                isNew ? 'País creado exitosamente' : 'País actualizado exitosamente',
                'Éxito'
            );
    ...
    $('#btn-save').on('click', async () => {
        const form = document.getElementById('frm-country') as HTMLFormElement;
