// ============================================================================
// Archivo: countries-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/Countries/countries-dt.ts
// Descripción:
//   - Lista de Países con DataTables
...
(function () {
    ...
    const pageContainer = d.querySelector("#countries-page");
    ...
    const $table: any = $("#countries-table");
    if (!$table.length) return;

    let countriesData: CountryRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['RecID', 'CountryCode', 'Name', 'NationalityCode', 'NationalityName', 'CreatedOn'];
    ...
    const titleize = (field: string): string => {
        const translations: Record<string, string> = {
            ...
            'Name': 'Nombre',
            'NationalityCode': 'Código Nacionalidad',
    ...
    const getColumnsFromData = (sample: CountryRow): string[] => {
        if (!sample || typeof sample !== 'object') {
            return [...defaultColumns];
        }
        const excluded = new Set(['RowVersion']);
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: CountryRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: CountryRow[] }) => void) => {
    ...
    const loadCountries = async (): Promise<CountryRow[]> => {
        const url = `${apiBase}/Countries?pageNumber=1&pageSize=100`;
        const response: CountryResponse = await fetchJson(url);
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('countries-summary');
        if (summary) {
            summary.textContent = `${count} país${count !== 1 ? 'es' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const rows: CountryRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `Paises_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteCountry = async (recId: string): Promise<void> => {
        const url = `${apiBase}/Countries/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar país: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/Country/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/Country/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ...
            : `¿Está seguro de eliminar ${count} países?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteCountry(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('País(es) eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar país(es)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/Countries?pageNumber=1&pageSize=1`;
            const probe: CountryResponse = await fetchJson(probeUrl);
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "Countries", userRefRecID, dataareaId);
