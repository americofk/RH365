...
(function () {
    ...
    const $table: any = $("#taxes-table");
    if (!$table.length) return;

    let taxesData: TaxRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'TaxCode', 'Name', 'LedgerAccount', 'TaxStatus', 'ValidFrom', 'ValidTo'];
    ...
    const getColumnsFromData = (sample: TaxRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: TaxRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: TaxRow[] }) => void) => {
    ...
    const loadTaxes = async (): Promise<TaxRow[]> => {
    ...
    const exportToCSV = (): void => {
        ...
        const rows: TaxRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
    ...
    const deleteTax = async (recId: string): Promise<void> => {
        ...
        (w as any).ALERTS.confirm(
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    (w as any).ALERTS.ok('Impuesto(s) eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar impuesto(s)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            ...
            if (probe?.Data && Array.isArray(probe.Data) && probe.Data.length > 0) {
