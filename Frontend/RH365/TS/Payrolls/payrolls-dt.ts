// ============================================================================
// Archivo: payrolls-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/Payrolls/payrolls-dt.ts
// Descripción:
//   - Lista de Nóminas con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// ISO 27001: Visualización controlada de datos de nómina con trazabilidad
// ============================================================================
...
(function () {
    ...
    const pageContainer = d.querySelector("#payrolls-page");
    ...
    const $table: any = $("#payrolls-table");
    if (!$table.length) return;

    let payrollsData: PayrollRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'Name', 'PayFrecuencyName', 'ValidFrom', 'ValidTo', 'PayrollStatus', 'CreatedOn'];

    let gridViewsManager: any;
    let gridColumnsManager: any;

    // Mapa de frecuencias de pago según GlobalsEnum
    const payFrecuencyMap: Record<number, string> = {
        0: 'Diario',
        1: 'Semanal',
        2: 'Bisemanal',
        3: 'Quincenal',
    ...
    const titleize = (field: string): string => {
        const translations: Record<string, string> = {
            ...
            'PayFrecuencyName': 'Frecuencia de Pago',
            'ValidFrom': 'Válido Desde',
            'ValidTo': 'Válido Hasta',
            'Description': 'Descripción',
            'IsRoyaltyPayroll': 'Nómina de Regalías',
            'IsForHourPayroll': 'Nómina por Hora',
            'BankSecuence': 'Secuencia Bancaria',
            'CurrencyRefRecID': 'ID Moneda',
            'CurrencyName': 'Moneda',
            'PayrollStatus': 'Estado',
    ...
    const formatCell = (value: unknown, field: string): string => {
        ...
        if (field === "PayFrecuencyName" || field === "PayFrecuency") {
            if (typeof value === "number") {
                return payFrecuencyMap[value] || String(value);
            }
            return String(value);
        }
        
        if (typeof value === "boolean") {
            if (field === "PayrollStatus") {
                return value ? '<span class="label label-success">Activo</span>' : '<span class="label label-danger">Inactivo</span>';
            }
            if (field === "IsRoyaltyPayroll" || field === "IsForHourPayroll") {
    ...
    const getColumnsFromData = (sample: PayrollRow): string[] => {
        if (!sample || typeof sample !== 'object') {
            return [...defaultColumns];
        }
        const excluded = new Set(['RecID', 'RowVersion', 'DataareaID', 'CurrencyRefRecID']);
        const columns = Object.keys(sample).filter(col => !excluded.has(col));
        
        // Agregar PayFrecuencyName si existe PayFrecuency
        if (columns.includes('PayFrecuency') && !columns.includes('PayFrecuencyName')) {
            const index = columns.indexOf('PayFrecuency');
            columns.splice(index, 1, 'PayFrecuencyName');
        }
        
        return columns;
    };

    const enrichPayrollData = (payrolls: any[]): PayrollRow[] => {
        return payrolls.map(payroll => {
            const enriched = { ...payroll };
            
            // Agregar PayFrecuencyName basado en PayFrecuency
            if (typeof payroll.PayFrecuency === 'number') {
                enriched.PayFrecuencyName = payFrecuencyMap[payroll.PayFrecuency] || `Código ${payroll.PayFrecuency}`;
            }
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: PayrollRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: PayrollRow[] }) => void) => {
                loadPayrolls().then(items => {
                    payrollsData = items;
                    callback({ data: items });
    ...
    const loadPayrolls = async (): Promise<PayrollRow[]> => {
        const url = `${apiBase}/Payrolls?skip=0&take=100`;
        const response = await fetchJson(url);
        if (Array.isArray(response)) {
            return enrichPayrollData(response);
        }
        throw new Error('Respuesta del API inválida');
    };

    const updateSummary = (count: number): void => {
        const summary = d.getElementById('payrolls-summary');
        if (summary) {
            summary.textContent = `${count} nómina${count !== 1 ? 's' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const fileName = `Nominas_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deletePayroll = async (recId: string): Promise<void> => {
        const url = `${apiBase}/Payrolls/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar nómina: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/Payroll/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/Payroll/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ...
            : `¿Está seguro de eliminar ${count} nóminas?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deletePayroll(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Nómina(s) eliminada(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar nómina(s)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/Payrolls?skip=0&take=1`;
            const probe = await fetchJson(probeUrl);
            if (Array.isArray(probe) && probe.length > 0) {
                const enrichedProbe = enrichPayrollData(probe);
                allColumns = getColumnsFromData(enrichedProbe[0]);
            } else {
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "Payrolls", userRefRecID, dataareaId);
