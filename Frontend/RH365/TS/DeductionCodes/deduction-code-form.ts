...
(function () {
    ...
    interface FieldConfig {
        field: string;
        label: string;
        type: 'text' | 'number' | 'date' | 'datetime' | 'textarea' | 'select' | 'checkbox' | 'decimal';
        required?: boolean;
        maxLength?: number;
        min?: number;
        max?: number;
    ...
    let deductionCodeData: any = null;
    let projectOptions: { value: string; text: string }[] = [];
    let categoryOptions: { value: string; text: string }[] = [];
    ...
    const loadCategoryOptions = async (projectRecID?: number): Promise<void> => {
        try {
            ...
            categoryOptions = filteredData.map((item: any) => ({
                value: item.RecID?.toString() || '',
    ...
    // ========================================================================
    const getGeneralFields = (): FieldConfig[] => [
        // COLUMNA IZQUIERDA
        {
            field: 'Name',
            label: 'Nombre',
    ...
    // CAMPOS CONTRIBUCIÓN
    // ========================================================================
    const contributionFields: FieldConfig[] = [
        {
    ...
    const clearFieldValue = ($field: any, config: FieldConfig): void => {
    ...
    const toggleContributionFields = (enable: boolean): void => {
    ...
    const toggleDeductionFields = (enable: boolean): void => {
    ...
    const applyPayrollActionLogic = (): void => {
    ...
    const loadDeductionCodeData = async (): Promise<void> => {
        await Promise.all([
            loadProjectOptions(),
            loadCategoryOptions(),
        ...
        try {
            ...
            deductionCodeData = await fetchJson(url);
            ...
            (w as any).ALERTS.error('Error al cargar los datos', 'Error');
            renderGeneralForm({});
            renderContributionForm({});
            renderDeductionForm({});
            renderAuditForm({});
    ...
    const renderGeneralForm = (data: any): void => {
    ...
    const renderContributionForm = (data: any): void => {
    ...
    const renderDeductionForm = (data: any): void => {
    ...
    const saveDeductionCode = async (): Promise<void> => {
        ...
        try {
            ...
            (w as any).ALERTS.ok(
                isNew ? 'Creado exitosamente' : 'Actualizado exitosamente',
                'Éxito'
            );
    ...
    $('#btn-save').on('click', async () => {
        const form = document.getElementById('frm-deduction-code') as HTMLFormElement;
        if (!form.checkValidity()) {
            form.reportValidity();
            return;
        }
        await saveDeductionCode();
    });
    ...
    $(async function () {
        try {
            await loadDeductionCodeData();
        } catch (error) {
            (w as any).ALERTS.error('Error al inicializar', 'Error');
        }
    });
