// ============================================================================
// Archivo: employees-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/Employees/employees-dt.ts
// Descripción:
//   - Lista de Empleados con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// Estándar: ISO 27001 - Trazabilidad de consultas y accesos
// ============================================================================
...
(function () {
    ...
    const pageContainer = d.querySelector("#employees-page");
    ...
    const $table: any = $("#employees-table");
    if (!$table.length) return;

    let employeesData: EmployeeRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['EmployeeCode', 'Name', 'LastName', 'Gender', 'MaritalStatus', 'EmployeeStatus', 'AdmissionDate'];
    ...
    const titleize = (field: string): string => {
        const translations: Record<string, string> = {
            ...
            'ApplyForOvertime': 'Horas Extras',
            'IsFixedWorkCalendar': 'Horario Fijo',
    ...
    const getGenderLabel = (value: number): string => {
        const genderMap: Record<number, string> = {
            0: '<span class="label label-info">Masculino</span>',
            1: '<span class="label label-warning">Femenino</span>',
            2: '<span class="label label-default">No especificado</span>'
        };
        return genderMap[value] || String(value);
    };

    const getMaritalStatusLabel = (value: number): string => {
    ...
    const getPayMethodLabel = (value: number): string => {
        const methodMap: Record<number, string> = {
            0: '<span class="label label-default">Efectivo</span>',
            1: '<span class="label label-primary">Transferencia</span>'
        };
        return methodMap[value] || String(value);
    };

    const getWorkStatusLabel = (value: number): string => {
        const statusMap: Record<number, string> = {
            0: '<span class="label label-warning">Candidato</span>',
            1: '<span class="label label-danger">Despedido</span>',
            2: '<span class="label label-success">Empleado</span>',
            3: '<span class="label label-default">Deshabilitado</span>'
        };
        return statusMap[value] || String(value);
    };
    ...
    const formatCell = (value: unknown, field: string): string => {
        ...
        if (field === "Gender") {
            return getGenderLabel(Number(value));
        }

        if (field === "MaritalStatus") {
            return getMaritalStatusLabel(Number(value));
        }

        if (field === "PayMethod") {
            return getPayMethodLabel(Number(value));
        }

        if (field === "WorkStatus") {
            return getWorkStatusLabel(Number(value));
        }
    ...
    const getColumnsFromData = (sample: EmployeeRow): string[] => {
        if (!sample || typeof sample !== 'object') {
            return [...defaultColumns];
        }
        const excluded = new Set(['RecID', 'RowVersion', 'DataareaID', 'CountryRecId', 'DisabilityTypeRecId', 'EducationLevelRecId', 'OccupationRecId']);
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: EmployeeRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: EmployeeRow[] }) => void) => {
                loadEmployees().then(items => {
                    employeesData = items;
                    callback({ data: items });
    ...
    const loadEmployees = async (): Promise<EmployeeRow[]> => {
        const url = `${apiBase}/Employees?pageNumber=1&pageSize=100`;
        const response: EmployeeResponse = await fetchJson(url);
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('employees-summary');
        if (summary) {
            summary.textContent = `${count} empleado${count !== 1 ? 's' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const rows: EmployeeRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `Empleados_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteEmployee = async (recId: string): Promise<void> => {
    ...
    $('#btn-new').on('click', () => { window.location.href = '/Employee/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/Employee/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ...
            : `¿Está seguro de eliminar ${count} empleados?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteEmployee(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Empleado(s) eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar empleado(s)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/Employees?pageNumber=1&pageSize=1`;
            const probe: EmployeeResponse = await fetchJson(probeUrl);
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "Employees", userRefRecID, dataareaId);
