...
(function () {
    ...
    const $table: any = $("#courses-table");
    if (!$table.length) return;

    let coursesData: CourseRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'CourseCode', 'Name', 'StartDate', 'EndDate', 'CourseStatus', 'CreatedOn'];
    ...
    const getColumnsFromData = (sample: CourseRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: CourseRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: CourseRow[] }) => void) => {
    ...
    const loadCourses = async (): Promise<CourseRow[]> => {
        try {
    ...
    const exportToCSV = (): void => {
        ...
        const rows: CourseRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
    ...
    const deleteCourse = async (recId: string): Promise<void> => {
        ...
        (w as any).ALERTS.confirm(
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    (w as any).ALERTS.ok('Curso(s) eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar curso(s)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            ...
            const probe: CourseResponse = await fetchJson(probeUrl);
