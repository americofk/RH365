// ============================================================================
// Archivo: departments-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/Departments/departments-dt.ts
// Descripción:
//   - Lista de Departamentos con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// Estándar: ISO 27001 - Seguridad en visualización de datos
// ============================================================================
...
(function () {
    ...
    const pageContainer = d.querySelector("#departments-page");
    ...
    const $table: any = $("#departments-table");
    if (!$table.length) return;

    let departmentsData: DepartmentRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'DepartmentCode', 'Name', 'QtyWorkers', 'DepartmentStatus', 'CreatedOn'];
    ...
    const titleize = (field: string): string => {
        const translations: Record<string, string> = {
            ...
            'Name': 'Nombre',
            'QtyWorkers': 'Cantidad Trabajadores',
            'StartDate': 'Fecha Inicio',
            'EndDate': 'Fecha Fin',
            'Description': 'Descripción',
            'DepartmentStatus': 'Estado',
    ...
    const getColumnsFromData = (sample: DepartmentRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: DepartmentRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: DepartmentRow[] }) => void) => {
                loadDepartments().then(items => {
    ...
    const loadDepartments = async (): Promise<DepartmentRow[]> => {
        ...
        try {
            ...
            // Si la respuesta es un array directo (no paginado)
            if (Array.isArray(response)) {
                console.log(`✓ ${response.length} departamentos cargados (array directo)`);
                return response;
            }

            // Si la respuesta tiene estructura paginada
            if (response?.Data && Array.isArray(response.Data)) {
                console.log(`✓ ${response.Data.length} departamentos cargados (paginado)`);
                return response.Data;
            }

            // Si no coincide con ninguna estructura esperada
            console.error('Estructura inesperada del API:', response);
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('departments-summary');
        if (summary) {
            summary.textContent = `${count} departamento${count !== 1 ? 's' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const rows: DepartmentRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `Departamentos_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteDepartment = async (recId: string): Promise<void> => {
        const url = `${apiBase}/Departments/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar departamento: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/Department/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/Department/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ...
            : `¿Está seguro de eliminar ${count} departamentos?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteDepartment(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Departamento(s) eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar departamento(s)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/Departments?pageNumber=1&pageSize=1`;
            const probe: DepartmentResponse = await fetchJson(probeUrl);
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "Departments", userRefRecID, dataareaId);
