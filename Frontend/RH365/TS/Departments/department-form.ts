...
(function () {
     ...
     */
    const loadDepartmentData = async (): Promise<void> => {
        if (isNew) {
            renderBusinessForm({});
            renderAuditForm({});
            return;
        ...
        try {
            ...
            departmentData = await fetchJson(url);

            renderBusinessForm(departmentData);
            renderAuditForm(departmentData);
        } catch (error) {
            (w as any).ALERTS.error('Error al cargar los datos del departamento', 'Error');
    ...
    const saveDepartment = async (): Promise<void> => {
        ...
        try {
            ...
            (w as any).ALERTS.ok(
                isNew ? 'Departamento creado exitosamente' : 'Departamento actualizado exitosamente',
                'Éxito'
            );
    ...
    $('#btn-save').on('click', async () => {
        const form = document.getElementById('frm-department') as HTMLFormElement;
