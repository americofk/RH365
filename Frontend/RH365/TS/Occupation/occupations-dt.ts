// ============================================================================
// Archivo: occupations-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/Occupations/occupations-dt.ts
// Descripción:
//   - Lista de Ocupaciones con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// ============================================================================

type OccupationRow = Record<string, unknown>;
...
(function () {
    ...
    const pageContainer = d.querySelector("#occupations-page");
    ...
    const $table: any = $("#occupations-table");
    if (!$table.length) return;

    let occupationsData: OccupationRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['OccupationCode', 'Description', 'CreatedOn', 'ModifiedOn'];
    ...
    const getColumnsFromData = (sample: OccupationRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: OccupationRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: OccupationRow[] }) => void) => {
                loadOccupations().then(items => {
    ...
    const loadOccupations = async (): Promise<OccupationRow[]> => {
        const url = `${apiBase}/Occupations?pageNumber=1&pageSize=100`;
        const response: OccupationResponse = await fetchJson(url);
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('occupations-summary');
        if (summary) {
            summary.textContent = `${count} ocupación${count !== 1 ? 'es' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const fileName = `Ocupaciones_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteOccupation = async (recId: string): Promise<void> => {
        const url = `${apiBase}/Occupations/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar ocupación: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/Occupation/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/Occupation/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ? '¿Está seguro de eliminar esta ocupación?'
            : `¿Está seguro de eliminar ${count} ocupaciones?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteOccupation(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Ocupación(es) eliminada(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar ocupación(es)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/Occupations?pageNumber=1&pageSize=1`;
            const probe: OccupationResponse = await fetchJson(probeUrl);
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "Occupations", userRefRecID, dataareaId);
