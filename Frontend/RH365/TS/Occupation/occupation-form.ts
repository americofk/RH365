...
(function () {
     ...
     */
    const loadOccupationData = async (): Promise<void> => {
        if (isNew) {
     ...
     */
    const saveOccupation = async (): Promise<void> => {
        ...
        try {
            ...
            (w as any).ALERTS.ok(
                isNew ? 'Ocupación creada exitosamente' : 'Ocupación actualizada exitosamente',
                'Éxito'
            );
    ...
    $('#btn-save').on('click', async () => {
        const form = document.getElementById('frm-occupation') as HTMLFormElement;
