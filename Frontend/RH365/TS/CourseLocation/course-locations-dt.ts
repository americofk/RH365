// ============================================================================
// Archivo: course-locations-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/CourseLocation/course-locations-dt.ts
// Descripción:
//   - Lista de Ubicaciones de Curso con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// ISO 27001: Gestión de datos con trazabilidad y auditoría
// ============================================================================

type CourseLocationRow = Record<string, unknown>;

interface CourseLocationResponse {
    Data: CourseLocationRow[];
    TotalCount: number;
    PageNumber: number;
    PageSize: number;
    TotalPages: number;
...
(function () {
    ...
    const pageContainer = d.querySelector("#courselocations-page");
    ...
    const $table: any = $("#courselocations-table");
    if (!$table.length) return;

    let courseLocationsData: CourseLocationRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'CourseLocationCode', 'Name', 'Description', 'CreatedOn'];
    ...
    const getColumnsFromData = (sample: CourseLocationRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: CourseLocationRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: CourseLocationRow[] }) => void) => {
                loadCourseLocations().then(items => {
                    courseLocationsData = items;
                    callback({ data: items });
    ...
    const loadCourseLocations = async (): Promise<CourseLocationRow[]> => {
        const url = `${apiBase}/CourseLocations?pageNumber=1&pageSize=100`;
        console.log('Cargando ubicaciones desde:', url);

        try {
            const response: CourseLocationResponse = await fetchJson(url);
            console.log('Respuesta recibida:', response);

            if (response && response.Data && Array.isArray(response.Data)) {
                console.log(`✓ ${response.Data.length} ubicaciones cargadas`);
                return response.Data;
            }

            // Si no hay Data pero la respuesta es un array directamente
            if (Array.isArray(response)) {
                console.log(`✓ ${response.length} ubicaciones cargadas (array directo)`);
                return response as CourseLocationRow[];
            }

            console.warn('Respuesta vacía o sin datos', response);
            return [];
        } catch (error) {
            console.error('Error en loadCourseLocations:', error);
            throw error;
        }
    };

    const updateSummary = (count: number): void => {
        const summary = d.getElementById('courselocations-summary');
        if (summary) {
            summary.textContent = `${count} ubicación${count !== 1 ? 'es' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        if (!courseLocationsData.length) {
            (w as any).ALERTS.warn('No hay datos para exportar', 'Advertencia');
            return;
        }
        const dt: any = $table.DataTable();
        const rows: CourseLocationRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `Ubicaciones_Curso_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteCourseLocation = async (recId: string): Promise<void> => {
    ...
    $('#btn-new').on('click', () => { window.location.href = '/CourseLocation/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/CourseLocation/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ...
            : `¿Está seguro de eliminar ${count} ubicaciones?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteCourseLocation(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Ubicación(es) eliminada(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar ubicación(es)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/CourseLocations?pageNumber=1&pageSize=1`;
            const probe: CourseLocationResponse = await fetchJson(probeUrl);
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "CourseLocations", userRefRecID, dataareaId);
