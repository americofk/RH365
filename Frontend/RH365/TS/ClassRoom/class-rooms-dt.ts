// ============================================================================
// Archivo: class-rooms-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/ClassRooms/class-rooms-dt.ts
// Descripción:
//   - Lista de Salones de Clases con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// ============================================================================

type ClassRoomRow = Record<string, unknown>;
...
(function () {
    ...
    const pageContainer = d.querySelector("#classrooms-page");
    ...
    const $table: any = $("#classrooms-table");
    if (!$table.length) return;

    let classRoomsData: ClassRoomRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'ClassRoomCode', 'Name', 'MaxStudentQty', 'AvailableTimeStart', 'CreatedOn'];

    let gridViewsManager: any;
    let gridColumnsManager: any;
    
    // Mapeo de ID de ubicación a nombre
    let courseLocationsMap: Record<number, string> = {};

    const titleize = (field: string): string => {
        const translations: Record<string, string> = {
            'RecID': 'ID Registro',
            'ID': 'ID',
            'ClassRoomCode': 'Código Salón',
            'Name': 'Nombre',
            'MaxStudentQty': 'Capacidad',
            'CourseLocationRefRecID': 'Ubicación',
            'Comment': 'Comentario',
            'AvailableTimeStart': 'Hora Inicio',
            'AvailableTimeEnd': 'Hora Fin',
    ...
    const formatCell = (value: unknown, field: string): string => {
        ...
        // Formatear CourseLocationRefRecID para mostrar el nombre
        if (field === "CourseLocationRefRecID" && typeof value === "number") {
            return courseLocationsMap[value] || `ID: ${value}`;
        }
    ...
    const getColumnsFromData = (sample: ClassRoomRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: ClassRoomRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: ClassRoomRow[] }) => void) => {
                loadClassRooms().then(items => {
                    classRoomsData = items;
                    callback({ data: items });
                    updateSummary(items.length);
                }).catch(err => {
                    console.error('Error cargando salones de clases:', err);
    ...
    const loadClassRooms = async (): Promise<ClassRoomRow[]> => {
        const url = `${apiBase}/ClassRooms?skip=0&take=100`;
    ...
    const loadCourseLocations = async (): Promise<void> => {
        try {
            ...
            const response = await fetchJson(url);
            if (Array.isArray(response)) {
                // Crear mapeo RecID -> Name
                response.forEach((loc: any) => {
                    courseLocationsMap[loc.RecID] = loc.Name || loc.CourseLocationCode || 'Sin nombre';
                });
            }
        } catch (error) {
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('classrooms-summary');
        if (summary) {
            summary.textContent = `${count} salón${count !== 1 ? 'es' : ''} de clase${count !== 1 ? 's' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const rows: ClassRoomRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `SalonesClases_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteClassRoom = async (recId: string): Promise<void> => {
        const url = `${apiBase}/ClassRooms/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar salón de clases: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/ClassRoom/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/ClassRoom/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ? '¿Está seguro de eliminar este salón de clases?'
            : `¿Está seguro de eliminar ${count} salones de clases?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteClassRoom(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Salón(es) de clases eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar salón(es) de clases', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            ...
            const probeUrl = `${apiBase}/ClassRooms?skip=0&take=1`;
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "ClassRooms", userRefRecID, dataareaId);
