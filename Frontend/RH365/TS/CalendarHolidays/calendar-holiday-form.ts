...
(function () {
    ...
    let calendarHolidayData: any = null;
     ...
     */
    const loadCalendarHolidayData = async (): Promise<void> => {
        if (isNew) {
        ...
        try {
            ...
            calendarHolidayData = await fetchJson(url);
            ...
            (w as any).ALERTS.error('Error al cargar los datos del día feriado', 'Error');
     ...
     */
    const saveCalendarHoliday = async (): Promise<void> => {
        ...
        try {
            ...
            (w as any).ALERTS.ok(
                isNew ? 'Día feriado creado exitosamente' : 'Día feriado actualizado exitosamente',
                'Éxito'
            );
    ...
    $('#btn-save').on('click', async () => {
        const form = document.getElementById('frm-calendar-holiday') as HTMLFormElement;
