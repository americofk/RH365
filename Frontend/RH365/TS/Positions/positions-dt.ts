// ============================================================================
// Archivo: positions-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/Positions/positions-dt.ts
// Descripción:
//   - Lista de Posiciones con DataTables
...
(function () {
    ...
    const pageContainer = d.querySelector("#positions-page");
    ...
    const $table: any = $("#positions-table");
    if (!$table.length) return;

    let positionsData: PositionRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['ID', 'PositionCode', 'PositionName', 'IsVacant', 'PositionStatus', 'StartDate', 'CreatedOn'];
    ...
    const formatCell = (value: unknown, field: string): string => {
        ...
        if (typeof value === "boolean") {
            ...
            if (field === "IsVacant") {
                return value ? '<span class="label label-warning">Vacante</span>' : '<span class="label label-info">Ocupada</span>';
            }
            return value ? "Sí" : "No";
        }
    ...
    const getColumnsFromData = (sample: PositionRow): string[] => {
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: PositionRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: PositionRow[] }) => void) => {
                loadPositions().then(items => {
                    positionsData = items;
                    callback({ data: items });
    ...
    const updateSummary = (count: number): void => {
        ...
        if (summary) {
            summary.textContent = `${count} posición${count !== 1 ? 'es' : ''}`;
        }
    };
    ...
    const exportToCSV = (): void => {
        ...
        const rows: PositionRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `Posiciones_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deletePosition = async (recId: string): Promise<void> => {
        const url = `${apiBase}/Positions/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar posición: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/Position/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/Position/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ? '¿Está seguro de eliminar esta posición?'
            : `¿Está seguro de eliminar ${count} posiciones?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deletePosition(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Posición(es) eliminada(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar posición(es)', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "Positions", userRefRecID, dataareaId);
