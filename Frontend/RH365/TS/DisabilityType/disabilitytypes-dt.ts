// ============================================================================
// Archivo: disabilitytypes-dt.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/DisabilityTypes/disabilitytypes-dt.ts
// Descripción:
//   - Lista de Tipos de Discapacidad con DataTables
//   - Gestión de vistas de usuario (UserGridViews)
//   - Genera columnas y filas dinámicamente desde API
// ============================================================================

type DisabilityTypeRow = Record<string, unknown>;

interface DisabilityTypeResponse {
    Data: DisabilityTypeRow[];
    TotalCount: number;
    PageNumber: number;
    PageSize: number;
    TotalPages: number;
...
(function () {
    ...
    const pageContainer = d.querySelector("#disabilitytypes-page");
    ...
    const $table: any = $("#disabilitytypes-table");
    if (!$table.length) return;

    let disabilityTypesData: DisabilityTypeRow[] = [];
    let allColumns: string[] = [];
    let visibleColumns: string[] = [];
    const defaultColumns: string[] = ['DisabilityTypeCode', 'Description', 'CreatedOn'];
    ...
    const getColumnsFromData = (sample: DisabilityTypeRow): string[] => {
        if (!sample || typeof sample !== 'object') {
            return [...defaultColumns];
        }
        const excluded = new Set(['RecID', 'RowVersion']);
    ...
    const initializeDataTable = (columns: string[]): void => {
        ...
        const dtConfig: any = {
            ...
            columns: [
                {
                    ...
                    render: (_data: any, _type: string, row: DisabilityTypeRow) => {
            ...
            ajax: (_data: any, callback: (result: { data: DisabilityTypeRow[] }) => void) => {
                loadDisabilityTypes().then(items => {
                    disabilityTypesData = items;
                    callback({ data: items });
                    updateSummary(items.length);
                }).catch(err => {
                    console.error('Error cargando tipos de discapacidad:', err);
    ...
    const loadDisabilityTypes = async (): Promise<DisabilityTypeRow[]> => {
        const url = `${apiBase}/DisabilityTypes?pageNumber=1&pageSize=100`;
        const response: DisabilityTypeResponse = await fetchJson(url);
    ...
    const updateSummary = (count: number): void => {
        const summary = d.getElementById('disabilitytypes-summary');
        if (summary) {
            summary.textContent = `${count} tipo${count !== 1 ? 's' : ''} de discapacidad`;
        }
    };
    ...
    const exportToCSV = (): void => {
        if (!disabilityTypesData.length) {
            (w as any).ALERTS.warn('No hay datos para exportar', 'Advertencia');
            return;
        }
        const dt: any = $table.DataTable();
        const rows: DisabilityTypeRow[] = dt.rows({ search: 'applied' }).data().toArray();
        if (!rows.length) {
        ...
        const fileName = `TiposDiscapacidad_${new Date().toISOString().slice(0, 10)}.csv`;
        link.href = url;
    ...
    const deleteDisabilityType = async (recId: string): Promise<void> => {
        const url = `${apiBase}/DisabilityTypes/${recId}`;
        const headers: Record<string, string> = { 'Authorization': `Bearer ${token}` };
        const response = await fetch(url, { method: 'DELETE', headers });
        if (!response.ok) {
            throw new Error(`Error al eliminar tipo de discapacidad: ${response.status}`);
        }
    };
    ...
    $('#btn-new').on('click', () => { window.location.href = '/DisabilityType/NewEdit'; });

    $('#btn-edit').on('click', () => {
        const $checked = $table.find('tbody input.row-check:checked').first();
        if ($checked.length) {
            const recId = $checked.data('recid');
            window.location.href = `/DisabilityType/NewEdit?recId=${recId}`;
        }
    });
    ...
    $('#btn-delete').on('click', async () => {
        ...
        const message = count === 1
            ...
            : `¿Está seguro de eliminar ${count} tipos de discapacidad?`;
            ...
            async (confirmed: boolean) => {
                ...
                try {
                    ...
                    $checked.each(function () {
                        ...
                        if (recId) {
                            promises.push(deleteDisabilityType(recId));
                        }
                    });
                    await Promise.all(promises);
                    (w as any).ALERTS.ok('Tipo(s) de discapacidad eliminado(s) correctamente', 'Éxito');
                    $table.DataTable().ajax.reload();
                } catch (error) {
                    console.error('Error al eliminar:', error);
                    (w as any).ALERTS.error('Error al eliminar tipo(s) de discapacidad', 'Error');
                }
            },
            { type: 'danger' }
        );
    ...
    $(async function () {
        try {
            const probeUrl = `${apiBase}/DisabilityTypes?pageNumber=1&pageSize=1`;
            const probe: DisabilityTypeResponse = await fetchJson(probeUrl);
            ...
            gridViewsManager = new GridViewsManagerClass(apiBase, token, "DisabilityTypes", userRefRecID, dataareaId);
