...
(function () {
    ...
    const loadRelatedEntities = () => __awaiter(this, void 0, void 0, function* () {
        try {
            const [projectsResp, departmentsResp] = yield Promise.all([
            ...
            if (Array.isArray(projectsResp)) {
                projectsOptions = projectsResp.map((p) => ({
                    value: p.RecID.toString(),
                    text: p.Name || p.ProjectCode
                }));
            }
            else if (projectsResp === null || projectsResp === void 0 ? void 0 : projectsResp.Data) {
                projectsOptions = projectsResp.Data.map((p) => ({
                    value: p.RecID.toString(),
                    text: p.Name || p.ProjectCode
                }));
            }
            ...
            if (Array.isArray(departmentsResp)) {
                departmentsOptions = departmentsResp.map((d) => ({
                    value: d.RecID.toString(),
                    text: d.Name || d.DepartmentCode
                }));
            }
            else if (departmentsResp === null || departmentsResp === void 0 ? void 0 : departmentsResp.Data) {
                departmentsOptions = departmentsResp.Data.map((d) => ({
                    value: d.RecID.toString(),
                    text: d.Name || d.DepartmentCode
                }));
            }
    ...
    const loadCategoryOptions = (projectRecID) => __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            categoriesOptions = filteredData.map((item) => {
                var _a;
                return ({
    ...
    const updateCategoryDropdown = (projectRecID) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (categoryField) {
            ...
            const optionsHtml = categoryField.options.map(opt => `<option value="${opt.value}">${opt.text}</option>`).join('');
    ...
    const loadEarningCodeData = () => __awaiter(this, void 0, void 0, function* () {
        yield loadRelatedEntities();
        if (isNew) {
            renderBusinessForm({});
            renderAuditForm({});
            return;
        ...
        try {
            ...
            earningCodeData = yield fetchJson(url);
            renderBusinessForm(earningCodeData);
            renderAuditForm(earningCodeData);
        }
        catch (error) {
            w.ALERTS.error('Error al cargar los datos del código de nómina', 'Error');
            renderBusinessForm({});
            renderAuditForm({});
        }
    });
    const renderBusinessForm = (data) => {
        ...
        const leftMainFields = businessFields.filter(config => config.column === 'left' && config.type !== 'checkbox');
        ...
        const rightMainFields = businessFields.filter(config => config.column === 'right' && config.type !== 'checkbox');
        ...
        const leftCheckboxFields = businessFields.filter(config => config.column === 'left' && config.type === 'checkbox');
        ...
        const rightCheckboxFields = businessFields.filter(config => config.column === 'right' && config.type === 'checkbox');
    ...
    const saveEarningCode = () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            w.ALERTS.ok(isNew ? 'Código de nómina creado exitosamente' : 'Código de nómina actualizado exitosamente', 'Éxito');
            //setTimeout(() => {
    ...
    $('#btn-save').on('click', () => __awaiter(this, void 0, void 0, function* () {
        const form = document.getElementById('frm-earning-code');
        if (!form.checkValidity()) {
            form.reportValidity();
            return;
        }
        yield saveEarningCode();
    }));
    $(function () {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield loadEarningCodeData();
            }
            catch (error) {
...
//# sourceMappingURL=earning-code-form.js.map