// ============================================================================
// Archivo: calendar-holiday-form.ts
// Proyecto: RH365.WebMVC
// Ruta: TS/CalendarHolidays/calendar-holiday-form.ts
// Descripción: 
//   - Formulario dinámico para Crear/Editar Días Feriados
...
//   - Integración con API REST (/api/CalendarHolidays)
...
(function () {
    ...
    const pageContainer = d.querySelector("#calendar-holiday-form-page");
    ...
    const businessFields = [
        // COLUMNA IZQUIERDA
        {
            ...
            type: 'date',
            required: true,
            helpText: 'Fecha del día feriado',
            column: 'left'
        },
        {
            field: 'Description',
    ...
    // Variable global para almacenar los datos del día feriado cargados desde el API
    let calendarHolidayData = null;
    // ========================================================================
    ...
    // ========================================================================
    // CARGA DE DATOS DEL DÍA FERIADO
    // ========================================================================
    /**
     * Carga los datos del día feriado desde el API (solo si es edición).
     * En modo creación, renderiza los formularios vacíos.
     */
    const loadCalendarHolidayData = () => __awaiter(this, void 0, void 0, function* () {
        if (isNew) {
        ...
        try {
            ...
            calendarHolidayData = yield fetchJson(url);
            // Renderizar ambos formularios con los datos cargados
            renderBusinessForm(calendarHolidayData);
            renderAuditForm(calendarHolidayData);
        }
        catch (error) {
            w.ALERTS.error('Error al cargar los datos del día feriado', 'Error');
     ...
     * @param data Datos del día feriado a mostrar
     */
        ...
        if (isNew) {
            container.html(`
                <div class="alert alert-warning" role="alert">
                    ...
                    Los campos de auditoría se generarán automáticamente después de guardar el día feriado.
                </div>
            `);
            return;
        }
    ...
    // ========================================================================
    // GUARDADO DE DÍA FERIADO
    // ========================================================================
    /**
     * Guarda el día feriado en el API (POST para crear, PUT para actualizar).
     * Muestra alertas de éxito o error y redirige al listado si es exitoso.
     */
    const saveCalendarHoliday = () => __awaiter(this, void 0, void 0, function* () {
        const formData = getFormData();
        try {
            // Determinar URL y método según el modo (crear/editar)
            const url = isNew ? `${apiBase}/CalendarHolidays` : `${apiBase}/CalendarHolidays/${recId}`;
            const method = isNew ? 'POST' : 'PUT';
            // Construir payload con los campos necesarios
            const payload = {
                CalendarDate: formData.CalendarDate,
            ...
            w.ALERTS.ok(isNew ? 'Día feriado creado exitosamente' : 'Día feriado actualizado exitosamente', 'Éxito');
            //// Redirigir al listado después de 1.5 segundos
            //setTimeout(() => {
            //    window.location.href = '/CalendarHoliday/LP_CalendarHolidays';
            //}, 1500);
        }
        catch (error) {
            console.error('Error al guardar:', error);
            let errorMessage = 'Error al guardar el día feriado';
    ...
    $('#btn-save').on('click', () => __awaiter(this, void 0, void 0, function* () {
        const form = document.getElementById('frm-calendar-holiday');
     ...
     * Carga los datos del día feriado y renderiza los formularios.
     */
    $(function () {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield loadCalendarHolidayData();
            }
            catch (error) {
...
//# sourceMappingURL=calendar-holiday-form.js.map